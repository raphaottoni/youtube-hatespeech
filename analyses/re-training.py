...
for bias in policital_spectrum:
    ...
    # Load all chanes of that bias
    for file in os.listdir("../data/processed/captions/"+ bias + "/" ):
        if file.endswith(".txt"):
            with open("../data/processed/captions/"+ bias+ "/" + file , "r") as file_reader:
                data = []
                ...
                # Create the directory to hold the model if it is needed
                if not os.path.exists("../models/biases/captions/" +bias):
                    os.makedirs("../models/biases/captions/" + bias)
                model_wiki.save("../models/biases/captions/"+ bias+ "/" + file[:-4] +".model")
